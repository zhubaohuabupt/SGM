				...
				for (int d = 0; d < DMAX; d++)
				{
						...
						{  
						   ...
						   for(int i=-WindowSize/2;i<=WindowSize/2;i++)
							...
							//   float righttmp1 = (src2[(row+i)*width + col+j-1-d]+right)/2;
							//	float righttmp2 = (src2[(row+i)*width + col+j+1-d]+right)/2;
							///*	float rightmax = max(max(righttmp1,righttmp2),right);
							//   float rightmin = min(min(righttmp1,righttmp2),right);
							//   float DiffLR=max( max(left-rightmax,rightmin-left),0.0f);*/
							// float DiffLR=min(min( abs(left-righttmp1),abs(left-righttmp2)),abs(left-right));

						 // 
							//   float lefttmp1 = (src1[(row+i)*width + col+j-1]+left)/2;
							//	float lefttmp2 = (src1[(row+i)*width + col+j+1]+left)/2;
							//	/*float leftmax = max(max(lefttmp1,lefttmp2),left);
							//   float leftmin = min(min(lefttmp1,lefttmp2),left);
							//   float DiffRL=max( max(right-leftmax,leftmin-right),0.0f);*/
       //                    float DiffRL=min(min( abs(right-lefttmp1),abs(right-lefttmp2)),abs(right-left));
							//   float sampledif=min(DiffLR,DiffRL);
